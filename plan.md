...
Cross-profile calculation evidence update on 2026-06-08:
...
PCB footprint, net assignment, and DRC-rule update on 2026-06-08:

- Supported ESP32-S3 and STM32 PCB drafts now embed resolved KiCad `.kicad_mod` footprint bodies instead of only footprint-name shells.
- The board generator now emits a net table and pad-level net assignments from schematic `pinNets`, so duplicate USB-C VBUS/GND/CC/data pads are treated as intentional same-net pads instead of false shorts.
- The supported-profile PCB outline is now `160mm x 120mm`, and `.kicad_pro` now records board DRC rules including `min_through_hole_diameter: 0.2` and `min_hole_clearance: 0.15`.
- Official KiCad 10.0.3 PCB DRC now reports:
  - ESP32-S3: `1` violation, `53` unconnected items; remaining violation is `lib_footprint_mismatch:1` for `U2 RF_Module:ESP32-S3-WROOM-1`.
