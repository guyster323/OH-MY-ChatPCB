...
test('ESP32-S3 supported sensor profile expands requested SWD into ESP32 debug nets and full interfaces', async () => {
  ...
  try {
    ...
    assert.match(schematic, /\(lib_id "Regulator_Switching:TPS62177DQC"\)/);
    assert.match(schematic, /\(lib_id "Device:L"\)/);
    ...
    assert.match(schematic, /\(symbol "Regulator_Switching:TPS62177DQC"/);
    assert.match(schematic, /\(property "Description" "500mA Step-Down Converter with Sleep Mode, fixed 3\.3V Output Voltage, 4\.75-28V input voltage, WSON-10"/);
    assert.doesNotMatch(schematic, /\(symbol "Regulator_Linear:AMS1117-3\.3"/);
    assert.doesNotMatch(schematic, /\(symbol "Regulator_Linear:TC1262-33"/);
    assert.doesNotMatch(schematic, /\(extends "AP1117-15"\)/);
    assert.match(schematic, /\(symbol "Connector:USB_C_Receptacle_USB2\.0_16P"/);
    assert.match(schematic, /\(symbol "Device:L"/);
    ...
    assert.ok(refs.includes('C3'));
    assert.ok(refs.includes('L1'));
    ...
    assert.ok(result.review.residualRisks.some((risk) => /U1 TPS62177DQC/i.test(risk)));
    assert.ok(!result.review.residualRisks.some((risk) => /calculation-blocker: regulator-thermal-budget/i.test(risk)));
    assert.ok(result.review.residualRisks.some((risk) => /calculation-warning: .*buck-loss-estimate .*183mW/i.test(risk)));
...
function assertProductionEvidence(metadata, { mcuRef, mcuValue, debugRole }) {
  ...
  assert.equal(byRef.get('U1')?.value, 'TPS62177DQC');
  assert.equal(byRef.get('U1')?.role, '3v3-buck-regulator');
  assert.equal(byRef.get('J4')?.role, 'usb-c-connector');
  assert.equal(byRef.get('J7')?.role, debugRole);
  assert.equal(byRef.get('L1')?.value, '2.2uH');
  assert.equal(byRef.get('C3')?.value, '10uF');
  ...
  assert.equal(byId.get('regulator-topology-selection')?.status, 'pass');
  assert.match(byId.get('regulator-topology-selection')?.result ?? '', /buck/i);
  assert.equal(byId.get('buck-loss-estimate')?.status, 'warning');
  assert.match(byId.get('buck-loss-estimate')?.result ?? '', /183mW/);
  assert.equal(byId.get('regulator-thermal-budget')?.status, 'pass');
  assert.match(byId.get('regulator-thermal-budget')?.result ?? '', /0\.85W LDO/i);
}
